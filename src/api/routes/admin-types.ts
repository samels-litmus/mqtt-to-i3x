...
import { ObjectType } from '../../store/object-store.js';
...
interface ObjectTypeParams {
  elementId: string;
}

export async function registerAdminTypesRoutes(
  fastify: FastifyInstance,
  _opts: FastifyPluginOptions
): Promise<void> {
  // POST /admin/objecttypes - Create a new object type
  fastify.post<{ Body: CreateObjectTypeBody }>(
    '/admin/objecttypes',
    async (request, reply) => {
  ...
  fastify.get<{ Params: ObjectTypeParams }>(
    '/admin/objecttypes/:elementId',
    async (request, reply) => {
      const store = request.apiContext.store;
  ...
  fastify.put<{ Params: ObjectTypeParams; Body: UpdateObjectTypeBody }>(
    '/admin/objecttypes/:elementId',
    async (request, reply) => {
      const store = request.apiContext.store;
      const { elementId } = request.params;
  ...
  fastify.delete<{ Params: ObjectTypeParams }>(
    '/admin/objecttypes/:elementId',
    async (request, reply) => {
      const store = request.apiContext.store;
      const { elementId } = request.params;
